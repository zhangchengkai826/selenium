...
{
	// Keyframes are stored in time-ascending order, so last keyframe gives end time.
	float f = Keyframes.back().TimePos;

	return f;
}

void BoneAnimation::Interpolate(float timePos, XMFLOAT4X4& toParentTransform)const
{
	if (timePos <= Keyframes.front().TimePos)
	{
		...
		XMStoreFloat4x4(&toParentTransform, XMMatrixAffineTransformation(S, zero, Q, P));
	}
	...
	else
	{
		...
		{
			if (timePos >= Keyframes[i].TimePos && timePos <= Keyframes[i + 1].TimePos)
			{
...
void AnimationClip::Interpolate(float timePos, std::vector<XMFLOAT4X4>& toParentTransforms)const
{
	for (UINT i = 0; i < BoneAnimations.size(); ++i)
	{
		BoneAnimations[i].Interpolate(timePos, toParentTransforms[i]);
	}
}
...
{
	auto clip = mAnimationClips.find(clipName);
